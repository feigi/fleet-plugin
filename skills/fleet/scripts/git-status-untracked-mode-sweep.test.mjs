...
//
// Zero deps: `node --test skills/fleet/scripts/git-status-untracked-mode-sweep.test.mjs`.
 ...
 */
const ALLOWED = [
  { file: "skills/fleet/scripts/instruments.sh", fragment: 'status --porcelain -- "$set"' },
];
...
test("the sweep sees the scripts it is supposed to police", { skip: SKIP_WITHOUT_REPO }, () => {
  for (const f of [
    "skills/fleet/scripts/reap.sh",
    "skills/fleet/scripts/release-ticket.sh",
    "skills/fleet/scripts/worktree-audit.sh",
    "skills/fleet/scripts/no-undo-audit.sh",
    "skills/fleet/scripts/claim-ticket.sh",
    "skills/fleet/scripts/instruments.sh",
  ]) {
    assert.ok(
