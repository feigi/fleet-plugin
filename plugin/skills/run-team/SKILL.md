...
>
> So: prefix every `edit` and `read` path with `<abs-path>`, never
> `plugin/scripts/foo.mjs` on its own. When a diff and a read disagree, and
> after any edit you are unsure of, run `git -C <main-checkout> status
> --porcelain` — empty is the only clean answer. Recover by copying your
> content into the worktree first, then `git -C <main-checkout> restore --
> <path>`, naming the path; never a bare `git restore .` there, which would
> destroy a sibling's uncommitted work in the same tree.
...
> Recover by copying your content into the worktree first, then
> `git -C <main-checkout> restore -- <path>`, naming the path; never a bare
> `git restore .` there, which would destroy a sibling's uncommitted work.
>
...
> > resolve to your scratch path — compare resolved forms (`realpath`), since
> > `--show-toplevel` can report `/private/tmp/…` for a `/tmp` scratch dir on
> > macOS.
>
