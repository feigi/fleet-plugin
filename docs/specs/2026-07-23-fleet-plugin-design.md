...
| `reap.sh` | `[--apply]` | `{reaped[], kept[{branch,reason}]}` | exit 1 under `--apply` only, and in violation of the rule above: a failing `git worktree prune` aborts after the branches are already deleted and before the payload is printed, so the caller gets the code reserved for a verdict with neither verdict nor payload; exit 2 — more than one argument, not a repository, the fetch failed, or `${BASE_REF:-origin/main}` does not resolve. A kept branch is a finding at exit 0, refusals included — but exit 0 is not a closed account of what was safe to delete: the merged check reads a `git cherry` that failed as "no unmerged commits" and reaps the branch |
