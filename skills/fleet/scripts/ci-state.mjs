...
// moment a job is added, which is what happened to the figures this comment used
// to carry. The path is not named here either, for the same reason it is not
// hardcoded in the code — a fixed `.github/workflows/ci.yml` is the #111 defect
// discoverWorkflowFile exists to fix.
//
// Not built from the fleet's own prose instead, because the prose does not name
// every job a workflow defines: `integration-docker` — a job in the agent-brain
// repo's CI workflow, which is on an internal GHE host and so cannot be settled
// from this repo — appears in no fleet document (`git grep -l integration-docker
// -- 'skills/fleet/**/*.md'` matches nothing), so a list built from the documents
// would have silently accepted a run missing it.
