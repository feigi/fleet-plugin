#!/bin/bash
# PreToolUse hook: mid-turn recall before a high-consequence action.
#
# Covers the "about to act" class: shared-system writes and large memory
# updates, where the useful signal is the tool INPUT. PreToolUse fires on every
# tool call, so is_high_consequence_action stays deliberately narrow — breadth
# here costs a round-trip per call. See
...
# and the PreToolUse envelope ONLY — all matching lives in the lib, enforced
...
is_high_consequence_action "$TOOL_NAME" "$TOOL_INPUT" || exit 0

# Query construction lives in the lib, mirroring the failure side.
QUERY=$(extract_action_query "$TOOL_INPUT") || exit 0
[[ -z "$QUERY" ]] && exit 0
