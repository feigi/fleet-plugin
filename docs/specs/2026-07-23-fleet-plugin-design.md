...
|---|---|---|---|
| `ci-state.mjs` | `--pr [--base --workflow --workflow-file --quiet]` | `{rid, attempt, headSha, branchHead, prHead, status, jobs[], missing[], behind, verdict}` | exit 1 not bound-green: head mismatch, run incomplete, expected job absent, any job not `success` — and, unguarded, a `gh` reply that parses as JSON but is not the shape asked for, which throws and lands on 1 rather than 2; exit 2 the question could not be answered — bad or missing `--pr`, no such PR, a `gh` query failed, `gh` output that does not parse as JSON, or a workflow file it cannot derive jobs from (unreadable, a job-level `name:`, or zero jobs). A failed `git` is not one: the behind-count probe is deliberately fail-soft, reports `behind: null`, and never changes the exit |
