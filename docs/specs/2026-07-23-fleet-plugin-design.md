...
| `verify-sha.sh` | `<branch> <sha>` | `{branch, sha, reachable, tip}` | exit 1 the sha is not reachable on `origin/<branch>` — a `git merge-base --is-ancestor` that fails rather than answers is reported identically, as `reachable: false`; exit 2 the question could not be answered — bad argument, the fetch failed, `origin/<branch>` does not resolve after it, or the sha is not a commit object in this repository |
