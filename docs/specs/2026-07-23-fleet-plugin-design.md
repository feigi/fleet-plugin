...
| `reap.sh` | `[--apply]` | `{applied, reaped[], worktreesRemoved[], kept[{branch, reason}]}` | exit 2 only — more than one argument, an unrecognised argument (the value guard the count guard alone used to let past, so a mistyped `--apply` no longer reads as a clean dry run — #250), not a repository, `json.sh` missing or unreadable (the guard sits above the fetch, so nothing is deleted first), the fetch failed, `${BASE_REF:-origin/main}` does not resolve, or, under `--apply`, `git worktree prune` failed. That last one carries the payload the way `inflight.sh`'s exit 2 does: the record is printed before the prune runs, so it says "the branches were reaped and the housekeeping afterwards failed", never "nothing happened". It is also why no exit 1 remains — that failure used to reach `set -e` and abort mid-run, handing the caller the code reserved for a verdict from a script that has none (#265). A kept branch is a finding at exit 0, refusals included — and a `git cherry` that could not answer is one of them: it lands in `kept[]` as `cherry probe failed — cannot tell if merged: <git's own message>` and is never reaped, since the merged check is the only thing authorizing `-D` and a probe that did not answer authorizes nothing (#264). A refused `git worktree remove` is another, and it says what the refusal actually did: a non-zero exit is no proof the removal had no effect, since git can clear the admin entry and still fail. The registry is re-read and the reason distinguishes `registration intact` from `registration cleared` — the registration is the only thing that read measures, so neither phrase claims anything about what is or is not left on disk; a registry read that itself fails says so rather than guessing either. A failing `git branch -D` likewise names its own step and quotes git's message, newlines collapsed, instead of leaving the cause on the terminal (#391). The sweep continues past both — one refusal must not strand the remaining branches — this script deletes nothing on disk beyond what `git worktree remove` itself does, and the dry run is unchanged, since it prints `would remove worktree` without attempting the removal and so cannot foresee a refusal. Every string in the payload — branch names and reason strings alike — goes through `json.sh`'s `jstr`; a field that could not be rendered is reported as `null` rather than aborting a run that has already deleted branches (#119). A second sweep then covers the worktrees the `[gone]` walk above structurally cannot see: it locates a worktree by the `branch refs/heads/<name>` line the porcelain prints, so a worktree carrying no such line was never refused — it was never considered, and no `kept` entry said so while the branch was reaped and the directory stayed on disk (#381). What route leaves a fleet worktree detached is not recorded: an earlier draft blamed the merge bot's server-side rebase (#149) and that is measured false — the bot reported `path=rebase` for #969, #970 and #972 and a dry run straight after printed `would remove worktree` for all three, a line only an attached worktree reaches. The sweep is bounded to `*/.worktrees/*`, the fleet's own worktree home: unlike the branch sweep it has no `[gone]` upstream to read as evidence the PR merged, and branchless-clean-and-patch-equivalent equally describes a `git worktree add --detach` scratch checkout that was never a ticket, which unbounded it deleted (#985). Inside that home it decides each worktree on its own state, with `git cherry` against `${BASE_REF:-origin/main}` — never `merge-base --is-ancestor`, which answers no for a rebased-then-merged tip that is patch-equivalent rather than an ancestor. Its removals join `worktreesRemoved[]`, which names every worktree the run removed from either sweep so a removal no reaped branch accounts for is still in the record; its declines join `kept[]` with `branch` as `null`, since there is none to name. Those declines are `worktree <path> has an unresolvable HEAD` (git reports the null object id and no branch line for all four ways its admin `HEAD` can break, so nothing about the worktree can be decided — #179), `worktree <path> holds commits that exist nowhere else`, `worktree <path> has a git operation in progress (<state>)`, `worktree <path> is not a fleet worktree — outside .worktrees/`, `worktree <path> remove refused (<registration state>)` — which names the path because `branch` is `null` here and git's own message for a locked worktree carries none, so two refusals in one run would otherwise be byte-identical — plus the `dirty`, `.git` linkage, main-checkout and unknown-existence declines the branch sweep already emits. No `--ignored` decline: that probe guards a `.env` or a scratch note outside `.worktrees/`, and the bound above already refused everything there. The in-progress decline is the one git is no backstop for: measured, git 2.50.1 (Apple Git-155), `git worktree remove` WITHOUT `--force` removes a worktree holding an interrupted rebase, and one holding a bisect, at exit 0 — both detach and both leave `git status --porcelain` empty, so every other check passes them through and the sequencer state goes with the directory |
