...
//
//   (a) no script under `scripts/` derives the repo it OPERATES ON from its
//       own location — the pattern the OLD `instruments.sh` had (#1337) and
//       ADR 0003's own text names: a `git -C`/`git rev-parse` call rooted at
//       `dirname "$0"` (sh) or `import.meta.dirname`/`__dirname` (js).
//   (b) no prose under `skills/ commands/ agents/ workflows/` contains a
//       literal source path, harness cache path, or registry filename — the
//       exact defect that produced the 35 `~/dev/fleet-plugin/…` callsites
//       ADR 0003 documents and #1336 rewrites through the Resolver.
//
...
test(
  "no tracked production script under scripts/ derives the repo it operates on from its own location",
  { skip: SKIP_WITHOUT_REPO },
  () => {
    const files = tracked(ROOT, ["scripts/*"]).filter((f) => !f.endsWith(".test.mjs"));
...
// The ONLY licensed exception: the 35 callsites ADR 0003 documents
// (`~/dev/fleet-plugin/…`, script AND cross-doc references alike) — #1336's
// entire job is routing them through the Resolver and shrinking this to
// zero. No other skip exists; every other match anywhere is a defect.
const DEV_CHECKOUT_ALLOWLIST = /^~\/dev\/fleet-plugin\//;
const EXPECTED_ALLOWLISTED_COUNT = 35;

/**
 * `{ violations, allowlistedCount }` for one source text: `violations` is
 * every literal-pattern match NOT covered by the `~/dev/fleet-plugin/…`
 * allowlist, each `{ pattern, index }`; `allowlistedCount` is how many
 * `~/dev/` matches WERE covered by it.
 */
export function literalPathMatches(text) {
  ...
  for (const { name, re } of LITERAL_PATTERNS) {
    ...
    let m;
    while ((m = g.exec(text))) {
      if (name === "home-dev-checkout" && DEV_CHECKOUT_ALLOWLIST.test(text.slice(m.index))) {
        allowlistedCount++;
        continue;
      }
...
test(
  "no prose under skills/ commands/ agents/ workflows/ contains a literal source/harness/registry path "
  + "outside the counted ~/dev/fleet-plugin/ allowlist",
  { skip: SKIP_WITHOUT_REPO },
  () => {
    const files = tracked(ROOT, ["skills/*", "commands/*", "agents/*", "workflows/*"]);
    ...
    // COUNTED, not skipped: #1336 rewrites the 35 into Resolver calls, so
    // this MUST fall to exactly 0 once that lands — a count that only ever
    // goes down, never silently absorbed as "expected drift".
    assert.equal(
      allowlistedCount,
      EXPECTED_ALLOWLISTED_COUNT,
      "the counted ~/dev/fleet-plugin/ allowlist drifted — #1336 must shrink this to exactly 0, and nothing "
      + "should be ADDING to it in the meantime",
    );
  },
);
...
test("literalPathMatches: an unrelated sentence with no literal path is clean", () => {
  ...
  assert.equal(r.allowlistedCount, 0);
});
...
test("literalPathMatches: a ~/dev/fleet-plugin/ callsite counts as allowlisted, not a violation", () => {
  const r = literalPathMatches("Read `~/dev/fleet-plugin/scripts/instruments.sh` and run it.");
  assert.deepEqual(r.violations, []);
  assert.equal(r.allowlistedCount, 1);
});
