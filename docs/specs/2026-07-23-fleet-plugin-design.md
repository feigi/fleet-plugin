...
All eleven obey the three rules above. `board.mjs` is a twelfth row below and
the one exception, carried here for its exit codes rather than as an eleventh-
style script: the cockpit degrades where rule 1 fails closed — an unreachable
`gh` costs a panel, not the board — and it carries the previous board forward
rather than holding no state, which rule 3 forbids. Its design is
`2026-07-24-fleet-cockpit-design.md`; what belongs here is the contract a
caller of `board.mjs build` acts on.
...
with no verdict to report must never exit 1. Where a script needs a verdict those
meanings cannot carry, it mints a further code and states it in its own row.
A code minted for a FAULT is not that. It says the script itself broke, so no
question was answered and none of 0/1/2 describes what happened — a different
category from a verdict, stated in its own row the same way: `board.mjs`'s 70
is the only one today, and a fault prints a stack where a refusal prints one
line.

| Script | In | Out | Non-zero when |
|---|---|---|---|
| `ci-state.mjs` | `--pr [--base --workflow --workflow-file --declare-no-ci --quiet]` | `{pr, branch, prHead, runId, attempt, runHeadSha, status, conclusion, behind, verdict, reasons[]}`, and `jobs[]`/`missing[]` too — `--quiet` drops them because they would duplicate what `reasons` already states (a run is still bound and queried; `--quiet` only trims the payload), while a `no-ci` verdict drops them unconditionally, quiet or not, because no run is ever bound at all — there is nothing to report (#927) | exit 1 not bound-green: head mismatch, run incomplete, expected job absent, any job not `success`; also `verdict: no-ci` (no workflow files under `.github/workflows/`) without `--declare-no-ci`, since absence never reads as pass; exit 2 the question could not be answered — bad or missing `--pr`, no such PR, a `gh` query failed, `gh` output that does not parse as JSON, output that parses as JSON but is not the shape the caller is about to read — an error object where an array of runs is expected, a run view missing its jobs, a run-list row or job entry that is not an object (#269) — `git rev-parse --show-toplevel` unable to locate the repo root, `.github/workflows/` present but unreadable, workflow files present under names other than `--workflow` or two sharing it, or a workflow file it cannot derive jobs from (unreadable, a job-level `name:`, or zero jobs). A failed behind-count `git` probe is not one: that probe is deliberately fail-soft, reports `behind: null`, and never changes the exit. One exit-2 cause names itself in the payload rather than on stderr alone: a `gh` read refused by the API rate limit prints `{pr, verdict: "rate-limited", reasons[]}` before dying, so a caller can tell a quota outage that clears on its own from a repo or workflow that stays unreadable. It reports no CI state it never read — `status`, `jobs` and `missing` are absent, not null — and every other exit-2 cause still prints nothing |
| `pr-overlap.mjs` | `--a --b` | `{a, b, files[], modules[], dirs[], prose[], proseUnrun, signal}` | exit 2 only — bad usage, or a query failed and the overlap is therefore unknown; every verdict, `none` included, is exit 0: it reports, the model rules. The fourth signal (#705) is the one deliberate exception to that first clause: `prose[]` is derived from a full `gh pr diff` of each side, which has a real ENOBUFS cliff on an append-only data PR, and it is ranked last — so a scan that cannot run reports itself in `proseUnrun` and leaves the other three answerable rather than taking the whole tool to exit 2. `proseUnrun` is `null` exactly when `prose[]` is a complete answer; non-null there means an empty array is silence, not a clear |
| `candidates.mjs` | `[--require-label L] [--allow-fallback] [--limit N]` | `[{n,t,l,d}]` | exit 1 the query succeeded and no row came back at all — an empty queue, not an error; exit 3 the query succeeded and rows came back, but the to-spec filter removed every one — an empty queue whose emptiness is the filter's doing rather than the query's, so a caller reports specs awaiting to-tickets instead of no work (#64); under `--allow-fallback` this judges the final pass alone, so a labeled pass the filter emptied that falls back to a genuinely empty unfiltered pass is exit 1; exit 2 the query cannot be trusted — malformed invocation, `--allow-fallback` without `--require-label`, `gh` failed, output that is not the shape the `--jq` reduction produces, or a result landing exactly at `--limit` and therefore possibly truncated |
| `inflight.sh` | `<N>` | `{issue, taken, hits[], unknown[], evidence}` | exit 1 the ticket is taken; exit 2 the question could not be answered — a probe that could not look is recorded in `unknown` and the other probes still run, so exit 2 from a probe failure carries the same payload as 0 and 1; a bad argument, no such issue, not a repository, `json.sh` or `net.sh` missing or unreadable — the guard fires before any probe, since exit 1 out of this script means `taken` — or the verdict could not be written still exit 2 with no payload at all |
| `verify-sha.sh` | `<branch> <sha>` | `{branch, sha, reachable, tip}` | exit 1 the sha is not reachable on `origin/<branch>`; exit 2 the question could not be answered — bad argument, the fetch failed or did not finish inside its budget and was killed (#347), `origin/<branch>` does not resolve after it, the sha is not a commit object in this repository, a `merge-base --is-ancestor` that failed rather than answered, `json.sh` or `net.sh` missing or unreadable, or a payload field that could not be escaped (#119) |
| `claim-ticket.sh` | `<N> <slug> <type> [--apply]` | `{issue, branch, worktree, install, ports{postgres, ollama}, runner, applied}` | exit 2 only, one code for every refusal — too few or too many arguments, an unknown argument in the `--apply` slot (#250, #804: the slot took a value check only at #804, and until then `--aply` was demoted to "not `--apply`" and ran as a dry run whose stdout and stderr were byte-identical to the flagless one, while a trailing argument past the flag was dropped and the run entered apply mode; the `--write-runner` branch was bounded in the same change, since its positional rewrite makes the claim path's arity check structurally unable to see a trailing argument that arrived on that branch), an issue that is not a number, not a repository, the worktree or the branch already exists, no lockfile to derive the install command from, `node` unusable where the dependency count needs it — probed by INVOCATION rather than by name, so it covers a name that does not resolve and equally an interpreter that resolves and will not run, which is the version-manager shim the capture's own comment already names — reported as the interpreter rather than as an unreadable manifest, because the capture merges whatever the failed interpreter wrote and reporting that under the manifest's name left an interpreter fault satisfying an assertion about an unparseable one (#1141) — no test command to stamp a runner from, which is also where the sibling derivation's own interpreter refusal arrives, `json.sh` missing or unreadable — the guard sits ahead of every mutation — a receipt field that could not be escaped (#119), and under `--apply` a failed label write, worktree add or install, or a lockfile the install mutated |
| `no-undo-audit.sh` | `<worktree> <branch>` | `{worktree, worktreeRewritten, branch, branchRewritten, clean, stash, conflicts[], conflictsRewritten[], atRisk[], atRiskRewritten[]}` | exit 1 the worktree is dirty — the stash count is reported, never gated; `stash` is `null` rather than a number in either of two shapes, neither of them a genuinely empty stash: the list comes back empty and that emptiness is not a genuinely empty stash — either `refs/stash` is not absent (an unreadable ref, an unreadable reflog, or a ref pointing at a missing object), or it is absent while its reflog is not and still names entries no ref points at, or it is absent and the reflog's own path could not be resolved (an unsearchable ancestor directory), so whether entries survive it could not be asked at all (#570) — which is not the same claim as a genuinely empty stash and is no longer reported as one, or the list call itself exited nonzero, which needs no empty list at all: git printed the entries it could read and then said it could not finish, so what it printed is short by exactly what it refused to read and is not a count (#482); exit 2 the question is unanswerable — bad argument, no such worktree, a worktree git does not answer for (its linkage is broken, and git still answers at rc 0 — for the enclosing repo when the `.git` is gone, from another worktree's HEAD and index when it names that worktree's admin dir), `BASE_REF` is not spelled `origin/<branch>` or `refs/remotes/<path>` (#1565), `BASE_REF` names the audited branch (#1565), a ref that does not resolve, a probe that could not run — with the stash reflog's path resolution the exception (#570): that one failing reports `unknown` on the payload at the verdict's own exit code rather than withholding the audit, the same rule `worktree`/`branch` follow below — a conflicting path no pathspec can name, `json.sh` missing, unreadable or failed to load, a conflicting-path or at-risk array that could not be escaped (#119) — and no payload is emitted, or the audit itself could not be written (#1472) — that one can fail after the payload has already begun printing, so stdout carries it truncated and unparseable, which that exit code and the named stderr line are what distinguish from a complete answer. `worktree` and `branch` are the exception (#431): they echo argv rather than reporting a finding, so an escaper that cannot render one reports that field and its `*Rewritten` flag as JSON `null` on the payload the audit already earned, at the verdict's own exit code. A `null` there is "this run could not render the path or branch you passed in", never "there is no worktree" and never a path |
| `prove-merge.sh` | `<pre> <post> <mergeCommit>` | `{proved, preIsAncestor, postIsAncestor, secondParent, firstParent, parentCount, proofPath, headWasCurrent, gates{}}` — the flat fields are OBSERVATIONS and `gates` is the verdict: `proved` is true exactly when every value in `gates` is true, so a consumer asserts all-true there and never infers load-bearing-ness from the flat fields (#18). Membership is per-invocation: `gates` is `{postIsAncestor, secondParentIsHead, exactlyTwoParents, headWasCurrent}` on the no-rebase path and adds `preDidNotLand` — leg 1, which reads true when the observation `preIsAncestor` reads false — on the rebase path. Leg 1's absence on the no-rebase path is why `preIsAncestor:true` can sit beside `proved:true` there without contradiction. `merge ∈ base` is a precondition rather than a gate and appears in neither: it dies at exit 2 with no payload, so it is already true wherever a payload exists at all | exit 1 any gate fails — the proof is a "no"; exit 2 the question could not be answered — bad argument, the fetch failed or did not finish inside its budget and was killed (#347), the base does not resolve, an argument is not a commit here, a `merge-base --is-ancestor` that failed rather than answered — which is where a base resolving to a non-commit lands, since only `pre`/`post`/`merge` are commit-checked — the merge is not reachable from the base, it has no second parent and is therefore not a merge, `json.sh` or `net.sh` is missing or unreadable, or a proof field could not be escaped (#119) |
| `reap.sh` | `[--apply]` | `{applied, reaped[], worktreesRemoved[], kept[{branch, reason}]}` | exit 2 only — more than one argument, an unrecognised argument (#250), not a repository, `json.sh`, `net.sh` or `worktree.sh` missing, unreadable or failed to load (all three guards sit above the fetch, so nothing is deleted first), `git rev-parse --show-toplevel` failed with an error that is neither of the two known “not in a worktree” answers, so the cwd-delete guard could not be verified (#1441), the fetch failed or did not finish inside its budget and was killed (#347), `BASE_REF must be a remote-tracking ref` — the accept-list this script had none of, and the precondition that makes qualifying the base to its `refs/remotes/` spelling sound rather than a guess (#924) — the qualified `${BASE_REF:-origin/main}` does not resolve, which is also how a base that resolves only as a local TAG named `origin/main` refuses here instead of answering the merge probe with the wrong commit (#924), or, under `--apply`, `git worktree prune` failed, quoting git's own captured message (#992) — that last one prints the record before the prune runs, so it says the branches were reaped and the housekeeping failed, never that nothing happened. No exit 1 remains (#265). A kept branch is a finding at exit 0, refusals included: `cherry probe failed — cannot tell if merged: <git's own message>` (#264), a refused `git worktree remove` whose reason distinguishes `registration intact` from `registration cleared` — the registration is the only thing that read measures, so neither phrase claims anything about what is left on disk — and a failing `git branch -D` quoting git's message (#391). The sweep continues past both. Every payload string goes through `json.sh`'s `jstr`; an unrenderable field is `null` (#119). A second sweep covers the worktrees the `[gone]` walk structurally cannot see: one carrying no `branch refs/heads/<name>` line was never refused, it was never considered (#381). What route leaves a worktree detached is deliberately not recorded — the merge bot's server-side rebase was named here and measured not to detach (#985). The sweep is bounded to `*/.worktrees/*`, the fleet's own worktree home: unlike the branch sweep it has no `[gone]` upstream to read as evidence the PR merged, and branchless-clean-and-patch-equivalent equally describes a scratch checkout that was never a ticket, which unbounded it deleted (#985). Inside that home it decides each worktree with `git cherry` against `${BASE_REF:-origin/main}` in its qualified `refs/remotes/` spelling (#924), never `merge-base --is-ancestor`, which answers no for a rebased-then-merged tip that is patch-equivalent rather than an ancestor. Its removals join `worktreesRemoved[]`, naming every worktree either sweep removed; its declines join `kept[]` with `branch` as `null`: `worktree <path> has an unresolvable HEAD` (#179), `worktree <path> holds commits that exist nowhere else`, `worktree <path> has a git operation in progress (<state>)` — git is no backstop for that one, since `worktree remove` without `--force` removes a worktree holding an interrupted rebase or a bisect at exit 0 (measured, git 2.50.1 Apple Git-155) — `worktree <path> is not a fleet worktree — outside .worktrees/`, and `worktree <path> remove refused (<registration state>)`, which names the path because `branch` is `null` here and git's message for a locked worktree carries none. Either sweep also refuses the worktree the run itself was started in — `worktree <path> holds the working directory this run was started in` — since removing it deletes the cwd every later git call needs, which turned a run whose every removal succeeded into a failed `worktree prune` at exit 2 (#992). Plus the `dirty`, `.git` linkage, main-checkout and unknown-existence declines the branch sweep already emits; no `--ignored` decline, since the bound refused everything that probe guards |
| `worktree-audit.sh` | — | `[{worktree, branch, ahead, dirty, dirtyFiles[], readable}]` | exit 2 only — any argument at all (#525), not a repository, `BASE_REF` does not name a remote-tracking ref (#1329), `${BASE_REF:-origin/main}` does not resolve, `json.sh` or `worktree.sh` is missing, unreadable or failed to load (both guards fire above the opening `[`, so nothing is emitted), the `git worktree list` the audit is assembled from could not be read (#551), or an entry could not be escaped (#119) — that one fires inside the emitting loop, so stdout carries the array truncated mid-element and unparseable, which the exit 2 and the named stderr line are what distinguish from a complete answer. A worktree that is dirty, missing on disk or unreadable is a finding at exit 0 |
| `ledger.mjs` | `row/filed/ruled/check/read/dispatch/settle/drain [--file <path>] [--require-file]` | one object per subcommand — `row` `{ticket, line, created}`, `filed` `{issue, subject, total}`, `ruled` `{pr, decision, total}`, `dispatch` `{member, ticket, line, created, total}`, `settle` `{member, outcome, ticket, line, changed}`, `drain` `{drain, created}` (#1799), `check` `{subject, found, match, ledger, verdict}`, and `near`/`nearTotal`/`tracker` besides unless this subject was already filed in this run, `read` the whole ledger as `{rows[], filed[], ruled[], dispatched[], drain}`, `drain` being the marker's reason or null. `ledger.ok` is false when there was no ledger file to read and when what was there did not parse as one, the peer of `tracker.ok` (#231). Both of `check`'s lists are capped, and both caps are named in the payload rather than left inferable (#154): `nearTotal` is how many scoring near-misses there were in all, the ones the top-3 `near` cap withheld included, while `tracker.truncated` only reports that gh returned more rows than `tracker.hits` shows, never how many, since gh reports no total `tracker.error` carries the failing directory in full and is otherwise unbounded — only the embedded git-failure cause is capped, at `CAUSE_MAX` (500 chars) via `cause()` (#940). | `check` alone reaches a verdict: exit 1 the subject was already filed in this run, exit 3 this run's ledger is clean but the tracker has matching issues that score against the subject — review before filing. Matching tracker issues that all score 0.00, and near-misses at or above `NEAR_SOFT_HIT`, reach `verdict: soft-hit` at exit 0 instead — rows to read before filing, never an established duplicate (#388). Exit 2 on any subcommand — bad usage, unknown subcommand, `--require-file` with no ledger file, or a ledger that cannot be read or written — and on a `dispatch`/`settle` whose record would be wrong: an unknown member or outcome, a member already dispatched, or settled as something else, and an implementer dispatch once the run is draining |
| `board.mjs` | `build/serve [--ledger <path>] [--prev <path>] [--port N] [--interval N] [--open] [--spend-since <epoch-ms>] [--spend-dir <path>]` | `build` prints the board model as JSON on stdout; `serve` atomically rewrites its workspace's `.fleet/board.json` every interval and serves it beside `board.html` until signalled, on a port derived from that workspace unless `--port` names one. The spend panel's transcript directory is resolved ONCE at startup and reused by every tick, so the panel cannot change session mid-run; `--spend-dir` names that directory outright, and where two sessions share one project directory the pinned one is whichever was newest at launch (#1583). Launching a second time for a workspace that already has a cockpit is a no-op that succeeds (#1585): a bind failure on a DERIVED port makes `serve` request `/board.json` from whoever holds it, with a ~1s timeout, and a payload whose `workspace` is ours means print that URL, start nothing, open nothing even under `--open` (#1714 — only a launch that binds a port opens a tab: `open` on macOS, else `xdg-open` then `wslview`, else a stderr warning naming the URL) and exit 0 — any other answer (another workspace, non-200, unparseable, silence) is a foreign holder it steps past, bounded inside the derived range. The payload carries `workspace` for exactly this, so no identity outlives the process publishing it | exit 2 every refusal, one code for all of them — a malformed, trailing or `=`-joined value on any flag above, `--open` given a value, an unknown flag, a stray positional, a missing or unknown subcommand, and `serve`'s own bind failure, which since #1585 is only an explicit `--port` that is taken (never scanned off, never handshaked) or a derived range with no free port left in it, that one naming every port it tried. There is no exit 1: the cockpit reports state and never rules on it, and a read it could not make is a degraded panel at exit 0 rather than a finding (#61/#169). exit 70 is an internal fault — the script broke, so it answered nothing and the vocabulary above does not apply: the top-level rejection handler prints a stack, or the rejected value rendered when what unwound was not an `Error`, and exits (#1093). It is `sysexits.h`'s `EX_SOFTWARE`, chosen because 0/1/2 are spent and 3 is `candidates.mjs`'s minted verdict |
...
though `pr-overlap.mjs` computes the four signals, because "a fired signal is
