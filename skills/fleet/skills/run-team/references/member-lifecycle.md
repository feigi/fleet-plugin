...
Specialist's report routes to *you*, controller; completed agent's final text is return value, lost if unconsumed. Relay is only path — reviewer has no channel to grandchild, and `SendMessage` to one returns `had no active task; resumed from transcript` without report. Four reviewers pinged, one run, ~15 specialists, 0 reports retrieved — every one of those reports had already reached controller.

So relay each: name source, send its *text* (ruling on it as "your finding" makes reviewer verify report it never sent, or act on one it cannot check), **require acknowledgement by source name**. Unacknowledged relay fails silently same way ping does. `review-pr-11` was sent `code-reviewer` report, msg `58c6bef2`, before its verdict; verdict read *"dispatched, never delivered. No report reached me and none was relayed. Dimension not covered by a specialist."* — then ruled that dimension solo, headline claim refuted by report it held. `review-pr-12` reported *"ALL FOUR DELIVERED NOTHING"* while its applied list mirrored the four relays item-for-item. Naming the gap honestly is necessary, not sufficient: relay sent ≠ relay read. Only controller holds msg id, so only controller detects discrepancy — re-send on it, hold verdict until acknowledged.
