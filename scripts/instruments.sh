...
# is APPENDED BY THE CONTROLLER MID-RUN, so a whole-repo set would fire the
# gate on the run's own bookkeeping every single time — a guard the controller
# learns to ignore, which is the failure the paragraph below names.
#
