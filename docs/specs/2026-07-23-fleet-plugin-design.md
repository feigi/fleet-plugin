...
| `verify-sha.sh` | `<branch> <sha>` | `{branch, sha, reachable, tip}` | exit 1 the sha is not reachable on `origin/<branch>`; exit 2 the question could not be answered — bad argument, the fetch failed, `origin/<branch>` does not resolve after it, the sha is not a commit object in this repository, or a `merge-base --is-ancestor` that failed rather than answered |
