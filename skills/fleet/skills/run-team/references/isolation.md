...
The runner is untracked and written once, at claim time, never rewritten — so a copy under `.worktrees/` is frozen at whenever that worktree was claimed, and reading or measuring one measures that build, not what the current emitter produces. Line 2 is what tells a reader which build they are holding: `# agent-test template: <cksum>`. Reproduce the number from the installed script (`cksum ~/.claude/skills/fleet/scripts/claim-ticket.sh`) or from any revision of it (`git show <rev>:skills/fleet/scripts/claim-ticket.sh | cksum`) and compare the first field — `cksum` also prints a byte count, and the path when it is given a file. It checksums the whole emitting script, not the runner template inside it, so it over-reports: a mismatch means this runner predates some change to `claim-ticket.sh` — a reworded die message or an edited comment moves it while the runner it produces stays byte-identical — not "the template changed". The error runs one way, so a match means fresh and a mismatch means "re-materialize to be sure", not "definitely stale". Nothing reads the stamp and nothing refuses on a mismatch; it makes staleness legible to a human, it gates nothing.
