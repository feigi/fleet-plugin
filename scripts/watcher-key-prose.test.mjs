...
const FLEET = REPO;
// The plugin root and the repo root are the same directory since the
// 2026-09-08 hoist, so FLEET above is still exactly "the plugin". The WALK
// below is not: it must stay the plugin's own component dirs, because the
// scope note further down is a measured claim about which paths this test
// does and does not police, and readdirSync(REPO) would silently annex
// docs/ and the repo root it names as out of scope.
...
// commands scripts skills`) is supposed to come back all-three-part. Deriving it also
...
// "Tree-wide" is WALK and nowhere else, deliberately: the same key restated at
// the repo root or under docs/ goes unflagged (measured — a copy at each of
// those two paths left this test green, while the identical string under the
// plugin's own dirs reddened it). Every copy #183 found lives here; widening the
// root is a different ticket.
test("no document under the plugin's own dirs states the two-part watcher key", () => {
  const offenders = WALK.flatMap((d) =>
    readdirSync(join(FLEET, d), { recursive: true, withFileTypes: true })
