...
const FLEET = REPO;
// The plugin root sits one level under the repo root since #1336's re-nest
// under `plugin/`; FLEET above walks up from THIS file's own location
// (`plugin/scripts/`), so it lands on the plugin root correctly regardless.
// The WALK below is still not `readdirSync(REPO)`: it must stay the plugin's
// own component dirs, because the scope note further down is a measured
