...
— `commands/`, `scripts/`, `skills/`, `agents/`, `workflows/` — the probes, the
libraries they source, and these runbooks, which you also read from that tree.
Not the whole repo: `docs/metrics/` is appended by THIS run, so covering it
would refuse on the run's own bookkeeping every time. It does not
...
0. **Once per run, before anything else.**
   ...
   **Pin the instruments, after that fast-forward and before anything reads
   them.** `~/dev/fleet-plugin/scripts/instruments.sh --pin` records what the
   scripts and runbooks in this repo say right now, which is what every
   ...
   **Launch the cockpit.** On the first phase-0 pass only:
   `node ~/dev/fleet-plugin/scripts/board.mjs serve --open &` in the
...
1. **Candidate scan** — `~/dev/fleet-plugin/scripts/candidates.mjs
...
3. **In-flight check** — `~/dev/fleet-plugin/scripts/inflight.sh <N>` per
...
4. **Read each survivor in full, once** — `gh issue view <N> --json title,body,comments
   ...
   **`staleness.mjs` runs that check for you, and answers in THREE values.**
   `~/dev/fleet-plugin/scripts/staleness.mjs --path <path> --gone
...
`~/dev/fleet-plugin/scripts/claim-ticket.sh <N> <slug> <type> --apply` does
...
```
~/dev/fleet-plugin/scripts/fleet-tick.mjs \
...
`~/dev/fleet-plugin/scripts/ci-state.mjs --pr <N>` per open PR already emits
...
>
> Read `~/dev/fleet-plugin/commands/review-and-fix.md` and run **steps 2, 3
...
`~/dev/fleet-plugin/commands/review-and-fix.md` — the file
...
`~/dev/fleet-plugin/commands/run-merge-bot.md`, run **one** pass, then
...
`~/dev/fleet-plugin/scripts/reap.sh --apply` recomputes every precondition
...
`~/dev/fleet-plugin/scripts/release-ticket.sh <N> <slug> <type> --apply` is the
...
**Verify every reported SHA.** `~/dev/fleet-plugin/scripts/verify-sha.sh
...
**Then.** **Re-pin first** — `~/dev/fleet-plugin/scripts/instruments.sh
...
`~/dev/fleet-plugin/scripts/worktree-audit.sh` — it reports each worktree's
...
`~/dev/fleet-plugin/scripts/ledger.mjs` subcommands (`row`, `filed`, `ruled`,
